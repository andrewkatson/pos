...
class GetUsersMatchingFragmentTests(PositiveOnlySocialTestCase):
    ...
    def test_blocked_by_user_excludes_from_search(self):
        """
        Tests that the user making the request is excluded from search results if they are blocked
        """
        ...
        # A blocks B.
        me = get_user_with_username(self.local_username)
        user_b = get_user_with_username(self.first_username)
        ...
        user_c = get_user_with_username(fields_c[Fields.username])
