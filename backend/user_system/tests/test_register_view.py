...
class RegisterTests(PositiveOnlySocialTestCase):
    ...
    def setUp(self):
        ...
        self.request = self.factory.post("/user_system/register")
    ...
    def test_invalid_username_returns_bad_response(self):
        # Test view register
        response = register(self.request, invalid_username, self.local_email, self.local_password, false, ip)
        self.assertEqual(response.status_code, FAIL)

    def test_invalid_email_returns_bad_response(self):
        # Create an instance of a POST request.
        # Test view register
        response = register(self.request, self.local_username, invalid_email, self.local_password, false, ip)
        self.assertEqual(response.status_code, FAIL)

    def test_invalid_password_returns_bad_response(self):
        # Test view register
        response = register(self.request, self.local_username, self.local_email, invalid_password, false, ip)
        self.assertEqual(response.status_code, FAIL)

    def test_invalid_remember_me_returns_bad_response(self):
        # Test view register
        response = register(self.request, self.local_username, self.local_email, self.local_password, invalid_bool, ip)
        self.assertEqual(response.status_code, FAIL)

    def test_invalid_ip_returns_bad_response(self):
        # Test view register
        response = register(self.request, self.local_username, self.local_email, self.local_password, false, invalid_ip)
        self.assertEqual(response.status_code, FAIL)

    def test_user_already_exists_returns_bad_response(self):
        # Test view register
        response = register(self.request, self.local_username, self.local_email, self.local_password, false, ip)
    ...
    def test_user_doesnt_exist_with_remember_me_returns_good_response(self):
        # Test view register
        response = register(self.request, self.local_username, self.local_email, self.local_password, true, ip)
        self.assertEqual(response.status_code, SUCCESS)
