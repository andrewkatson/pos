...
class RegisterTests(PositiveOnlySocialTestCase):
    ...
    def test_invalid_remember_me_returns_bad_response(self):
        # Test view register
        response = register(self.register_request, self.local_username, self.local_email, self.local_password, invalid_bool, ip)
        self.assertEqual(response.status_code, FAIL)

    def test_invalid_ip_returns_bad_response(self):
        # Test view register
        response = register(self.register_request, self.local_username, self.local_email, self.local_password, false, invalid_ip)
        self.assertEqual(response.status_code, FAIL)

    def test_user_already_exists_returns_bad_response(self):
        # Test view register
        response = register(self.register_request, self.local_username, self.local_email, self.local_password, false, ip)
