...
from ..views import get_user_with_username, get_comments_for_thread
...
class GetCommentsForThreadTests(PositiveOnlySocialTestCase):
    ...
    def setUp(self):
        ...
        self.get_comments_for_thread_request = self.factory.post("/user_system/get_comments_for_thread_request")

        # Recall that middleware are not supported. You can simulate a
        # logged-in user by setting request.user manually.
        self.get_comments_for_thread_request.user = get_user_with_username(self.local_username)

        # Also add a session
        middleware = SessionMiddleware(lambda req: None)
        middleware.process_request(self.get_comments_for_thread_request)
        self.get_comments_for_thread_request.session.save()
