...
class CommentOnPostTests(PositiveOnlySocialTestCase):
    ...
    def setUp(self):
        ...
        # Create an instance of a POST request.
        self.report_comment_request = self.factory.post("/user_system/report_comment")

        # Recall that middleware are not supported. You can simulate a
        # logged-in user by setting request.user manually.
        self.report_comment_request.user = get_user_with_username(self.local_username)

        # Also add a session
        middleware = SessionMiddleware(lambda req: None)
        middleware.process_request(self.report_comment_request)
        self.report_comment_request.session.save()
