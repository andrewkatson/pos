...
from .test_constants import username, email, password, ip, true, FAIL, SUCCESS, NOT_FOUND_REDIRECT
...
class LogoutUserTests(TestCase):
    ...
    def test_invalid_session_management_token_returns_bad_response(self):
        ...
        response = logout_user(self.request, invalid_session_management_token, self.series_identifier,
    ...
    def test_invalid_series_identifier_returns_bad_response(self):
        # Test view logout_user
        response = logout_user(self.request, self.session_management_token, invalid_series_identifier,
                               self.login_cookie_token)
        self.assertEqual(response.status_code, FAIL)

    def test_invalid_login_cookie_token_returns_bad_response(self):
        # Test view logout_user
        response = logout_user(self.request, self.session_management_token, self.series_identifier,
                               invalid_login_cookie_token)
        self.assertEqual(response.status_code, FAIL)
    ...
    def test_logged_in_user_logs_out(self):
        ...
        response = delete_user(self.request, self.session_management_token, self.series_identifier,
                               self.login_cookie_token)
