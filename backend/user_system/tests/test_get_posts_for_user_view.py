...
class GetPostsForUserTests(PositiveOnlySocialTestCase):
    ...
    def setUp(self):
        ...
        self.get_posts_for_user_request = self.factory.post("/user_system/get_posts_for_user_request")

        # Recall that middleware are not supported. You can simulate a
        # logged-in user by setting request.user manually.
        self.get_posts_for_user_request.user = get_user_with_username(self.local_username)

        # Also add a session
        middleware = SessionMiddleware(lambda req: None)
        middleware.process_request(self.get_posts_for_user_request)
        self.get_posts_for_user_request.session.save()
