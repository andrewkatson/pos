...
def login_user_google(request):
    ...
    # Omitting remember_me means "don't remember me" rather than being an error:
    # it is the only optional field here, and a client that never wants a
    # remembered device should not have to say so.
    if remember_me_str is None:
        remember_me = False
    else:
        try:
            remember_me = convert_to_bool(remember_me_str)
