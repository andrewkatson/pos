    ...
    private func makePost() {
        ...
        Task {
            ...
            do {
                ...
                if isTesting() {
                    userSession = try keychainHelper.load(UserSession.self, from: POSAppConstants.keychainService, account: "userSessionToken") ?? UserSession(sessionToken: "123", username: "test", userId: "", isIdentityVerified: false)
                } else {
                    guard let loaded = try keychainHelper.load(UserSession.self, from: POSAppConstants.keychainService, account: "userSessionToken") else {
