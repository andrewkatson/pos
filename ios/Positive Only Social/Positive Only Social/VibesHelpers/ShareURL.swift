...
enum ShareURL {
    ...
    /// backend registers nothing else), so a profile segment that isn't is not
    /// a profile we have a screen for. A predicate rather than a regex so
    /// Unicode letters count, as they do server-side.
