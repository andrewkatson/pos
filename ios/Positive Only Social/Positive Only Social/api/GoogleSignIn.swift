...
///
/// Checked in rather than injected, and safe to be: it ships inside every copy
/// of the app, so anyone can read it out of the IPA. PKCE, not secrecy, is what
/// protects the exchange — the same reason the Team ID and bundle ID sit in the
/// checked-in apple-app-site-association. It lives in source rather than the
/// Info.plist because the plist is generated by Xcode (GENERATE_INFOPLIST_FILE),
/// so there is no checked-in plist to hold it.
///
/// Blanking it is still the supported way to turn the feature off: an empty (or
/// whitespace) value makes GoogleSignInProvider report itself unconfigured and
/// no Google button is offered, the same "unset means the feature is off" rule
